...
export default async function handler(req,res){
 ...
 if(passed){
   ...
   if(existing)certificate=existing; else {const {data:created}=await db.from('certificates').insert({user_id:Number(s.sub),course_id:exam.course_id,exam_attempt_id:attempt.id,template_id:tpl?.id||null,certificate_code:code(),score}).select('id,certificate_code').single();certificate=created||null;}
 }
