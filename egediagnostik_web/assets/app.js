...
function updateTime(){
  const d=new Date(),clock=d.toLocaleTimeString('tr-TR',{hour12:false}),day=d.getDay(),minutes=d.getHours()*60+d.getMinutes(),open=day>=1&&day<=5&&minutes>=510&&minutes<1080;
  document.querySelectorAll('#liveClock,#clockLarge').forEach(x=>x.textContent=clock);
  const text=open?'AÇIK • 08:30–18:00':'KAPALI • Sonraki iş günü 08:30',w=document.querySelector('#workState'),wl=document.querySelector('#workLarge');
