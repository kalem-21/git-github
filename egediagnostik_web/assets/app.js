...
if(menu){menu.addEventListener('click',()=>body.classList.toggle('navopen'));}
...
const targets=document.querySelectorAll('.hero-panel,.pillars article,.value-grid article,.solution-grid article,.servicegrid article,.academy-list span,.contact,.content-split>*,.intro>*');
targets.forEach(el=>el.classList.add('reveal'));
if('IntersectionObserver'in window){const io=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add('is-visible');io.unobserve(entry.target);}});},{threshold:.12});targets.forEach(el=>io.observe(el));}else targets.forEach(el=>el.classList.add('is-visible'));
